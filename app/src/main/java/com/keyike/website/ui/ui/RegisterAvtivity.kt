...
class RegisterAvtivity:BaseActivity() {
    ...
    fun initListener(){
        ...
        btn_register.setOnClickListener {
            if(TextUtils.isEmpty(et_phone.text) || TextUtils.isEmpty(et_psw.text) || TextUtils.isEmpty(et_confrim_psw.text) || TextUtils.isEmpty(et_name.text) || TextUtils.isEmpty(et_code.text)){
            ...
            post("${BASE_URL}registerUser", hashMapOf<String,String>("token" to MyApplication.token!!,"mobile" to et_phone.text.toString(),"passwd" to et_psw.text.toString(),"confrim_passwd" to et_confrim_psw.text.toString(),"child_name" to et_name.text.toString(),"verify_code" to et_code.text.toString()),object : DataCallback<LoginBean>{
                override fun dataSuccess(result: LoginBean) {
                    if (result.code == 1) {
                        ...
                        Toast.makeText(this@RegisterAvtivity, "注册失败", Toast.LENGTH_LONG).show()
                    }
                }
        ...
        tv_timer.setOnClickListener {
            ...
            post("${BASE_URL}registerVerify", hashMapOf<String,String>("token" to MyApplication.token!!,"mobile" to et_phone.text.toString()),object :
                DataCallback<SmsCodeBean> {
                override fun dataSuccess(result: SmsCodeBean) {
                    if(result.code == 1){
                        Toast.makeText(this@RegisterAvtivity, "发送验证码成功", Toast.LENGTH_LONG).show()
                    }else{
                        Toast.makeText(this@RegisterAvtivity, "发送验证码失败", Toast.LENGTH_LONG).show()
                        tv_timer.text = "发送验证码"
                        timer?.cancel()
                        max_time = 0
                    }
